...
find . | grep "[0-9a-f]\{8\}-[0-9a-f]\{4\}-[0-9a-f]\{4\}-[0-9a-f]\{4\}-[0-9a-f]\{12\}" | while read LINE; do  echo $LINE | awk -F\/ '{print $NF}' | awk -F\. '{print $1}' | xargs -I {} grep {} $1 ; if [ $? -ne 0 ]; then  rm "$LINE"; fi; done
